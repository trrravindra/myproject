...
<div class="content">
	...
	<c:choose>
		...
		<c:when test="${isLoggedInUser}">
			...
			<spring:message code="caregiver.my.prescription" />
		</c:when>
		<c:otherwise>
			<h1 class="title">
				<spring:message
			...
			<div class="ctrl-wrap">
				...
				<c:set var="days30Filter"
					value="<%=ERefillConstants.LAST_30_DAYS %>" />
				<c:set var="days90Filter"
					value="<%=ERefillConstants.LAST_90_DAYS %>" />
				...
				<c:set var="medication"
					value="<%=ERefillConstants.MEDICATION_NAME %>" />


				<form class="uniform form-range-filter"
					action="${contextPath}/${locale}/caregiver/prescription/details"
					method="POST"
					data-cal-enable="${contextPath}/resources/images/erefill/icon-calendar.png">
					<input type="hidden" name="selectedPatientOid" value="" />
					<div class="show-range select-range">
						<!-- Show/select range form -->

						<label> <spring:message code="show.label" /> <!-- Form will auto-submit when value selected -->
							<select class="myPres-submit-on-change1" name="date_range"
							id="date_range">
								<c:choose>
									<c:when
						...
						</label> <label><spring:message code="my.prescriptions.from.label" />
							<input
							title="<spring:message code="my.prescriptions.from.label" />"
							type="text" class="date-from date-picker" id="from" name="from"
							value="${fromDate}"
							...
							title="<spring:message code="my.prescriptions.to.label" />"
							type="text" class="date-to date-picker" id="to" name="to"
							value="${toDate}"
							data-msg-required="<spring:message code="my.prescriptions.dates.required.errMsg" />"
							data-msg-malformed-date="<spring:message code="my.prescriptions.dates.format.errMsg" />"
							placeholder="<spring:message code="date.format"/>" maxlength="10" />
					...
					<div class="sorting">
						...
						<label> <spring:message code="sort.by.label" /> <!-- Form will auto-submit when value selected -->
							<select class="myPres-submit-on-change" id="sort_by"
							name="sort_by">
								<c:choose>
			...
			<form class="uniform form-prescriptions"
				...
				<div class="form-footer">
					<button type="submit"
						class="button-erefill green refill-submit no-click-button">
						<spring:message
							code="my.prescriptions.refill.selected.presc.label" />
					</button>
