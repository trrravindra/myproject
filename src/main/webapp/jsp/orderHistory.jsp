...
<div class="content">
	...
	<c:choose>
		<c:when test="${isLoggedInUser}">
			<h1 class="title">
				<spring:message code="my.order.history.title" />
			</h1>
		</c:when>
		<c:otherwise>
			<h1 class="title">
				<spring:message code="my.order.history.custodian.title" />
	...
	<c:choose>
		...
		<c:otherwise>
			...
			<c:if test="${filterDate eq 'All' }">
				<c:set var="datePrintVal" value="Toutes les days" />
			</c:if>
					...
					<%-- <c:forTokens
						...
						<c:choose>
							...
							<c:otherwise>
								<c:choose>
									<c:when test="${'All' eq stat}">
										<option value="${stat}">${stat} <spring:message code="status.print.value" /></option>
									</c:when>
									<c:otherwise>
