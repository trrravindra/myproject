...
<section class="sidenav">
	...
	<nav class="nav-general">
		<ul>
			<li><a
				class="<c:if test="${'myprescriptions' eq pageName}">active</c:if> no-click"
				href="${contextPath}/${locale}/prescription/details"> <c:if
						test="${empty selectedPatientOID || selectedPatientOID eq PatientOID }">
						<spring:message code="content.nav.my" />
					</c:if> <spring:message code="content.nav.prescription" /></a></li>
		</ul>
		<ul>
			<li><a
				class="<c:if test="${'myorderhistory' eq pageName}">active</c:if> no-click"
		...
		<ul>
			<li><a
				class="<c:if test="${'mymedicationrecord' eq pageName}">active</c:if> no-click"
		...
		<c:if
			...
			<ul>
				<li><a
					class="<c:if test="${'myaccount' eq pageName}">active</c:if> no-click"
					href="${contextPath}/${locale}/myaccount/details"><spring:message
							code="content.nav.account" /></a></li>
			</ul>
			<ul>
				<li><a
					class="<c:if test="${'mymanagedaccount' eq pageName}">active</c:if> no-click"
					href="${contextPath}/${locale}/managedaccount/details"><spring:message
							code="content.nav.managed.account" />
						<c:if
							test="${not empty pendingCustodianRequests && pendingCustodianRequests ne 0}">
							<span class="pendingreq"> (${pendingCustodianRequests})</span>
						</c:if></a></li>
			</ul>
		</c:if>
		<ul>
			<li><a
				class="<c:if test="${'help' eq pageName}">active</c:if> no-click"
				href="${contextPath}/${locale}/user/help"><spring:message
						code="content.nav.help" /></a></li>
		</ul>
		<ul>
			<li><a
				class="<c:if test="${'contactus' eq pageName}">active</c:if> no-click"
